...
plot_hdi <- function(traj, traj_infection){
  ...
  fig <- ggplot(data) +
    ...
    geom_point(aes(x = yearmon, y = count, shape = "Lab-confirmed cases\nfrom Scotland"), size = 2, colour = "black") +
    ...
    scale_shape_manual(name = "", values = c("Lab-confirmed cases\nfrom Scotland" = 16)) +
  ...
  ggsave(filename = here("output", "figures", "hdi", format(Sys.Date(), "%d%m%Y"), paste0(n, ".png")), plot = fig, width = 11, height = 5, dpi = 300)
