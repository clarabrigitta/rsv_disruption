...
create_data <- function(n_interest, rep, n_burn){
...
model_function <- function(lambda, theta1, theta2, omega1, omega2, alpha1, alpha2, stored_data, delta, n_interest){
  ...
  data <- map(1:nrow(babies),
          function(x){
            ...
            subdata[1, 1:(4+n_interest)] <- babies[x, ]
            subdata[, 1] <- x:(x+12*4-1)
            subdata[, 2] <- stored_data[[3]][x:(x+12*4-1)]
            subdata <- cbind(subdata,
                          waning = 1/(1 + exp(omega1 * (subdata[, 5+n_interest]-omega2))),
            ...
            for(month in 2:48){
              ...
              subdata[month - 1, 3:(3+n_interest)] <- subdata[month - 1, 3:(3+n_interest)] * subdata[month - 1, 2] * (1 - ((1 - start_inf) * subdata[month - 1, n_interest+6])) # calculate number of babies infected in each immunity level
              subdata[month - 1, n_interest+8] <- sum(subdata[month - 1, 3:(3+n_interest)]) # total number of infections at that time step
              ...
              subdata[month - 1, 3:(3+n_interest)] <- subdata[month - 1, 3:(3+n_interest)] * subdata[month - 1, n_interest+7] # calculate number of babies that develop disease in each immunity level
              subdata[month - 1, n_interest+9] <- sum(subdata[month - 1, 3:(3+n_interest)]) # total number of babies that develop disease at that time step
            }
