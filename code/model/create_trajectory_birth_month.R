...
create_trajectory_birth_month <- function(lambda, theta1, theta2, omega1, omega2, alpha1, alpha2, stored_data, delta, n_interest){
  ...
  women[303:315, 2] <-  women[303:315, 2] * lambda # period corresponding to March 2020 - March 2021
  # women[303:319, 2] <-  women[303:319, 2] * c(rep(0.01, 2), seq(0.01, lambda, length.out = 3), rep(lambda, 1), seq(lambda, 0.01, length.out = 5), rep(0.01, 2), seq(0.01, 1, length.out = 4)) # uncomment for alternative return to normal scenario
  ...
  stored_data[[3]][123:135] <- stored_data[[3]][123:135] * lambda # period corresponding to March 2020 - March 2021
  # stored_data[[3]][123:139] <-  stored_data[[3]][123:139] * c(rep(0.01, 2), seq(0.01, lambda, length.out = 3), rep(lambda, 1), seq(lambda, 0.01, length.out = 5), rep(0.01, 2), seq(0.01, 1, length.out = 4)) # uncomment for alternative return to normal scenario
  ...
  data <- do.call(rbind, data) # unlist map output, keep all columns, if ignoring immunity levels use [, c(1, 2, 29:34)]
