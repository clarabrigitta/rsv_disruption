...
duration = 60 # set duration of maternal immunity
...
likelihood_all <- function(param){
  ...
  # caseimport <- param[9]
  print(param)
  print(is.vector(param))

  likelihood <- dpois(round(scotland_rate$count, digits = 0),
                      as.numeric(round(model_function(lambda = exp(disruption), theta1 = inf_imm1, theta2 = inf_imm2, omega1 = waning1, omega2 = waning2, alpha1 = aging1, alpha2 = aging2, stored_data = save_data, delta = 0.015, n_interest = duration)[, 1], digits = 0)) * detection,
                      log = T)
...
# settings = list(iterations = 50000, nrChains = 1, message = TRUE, burnin = 25000, thin = 5) # for real runs
settings = list(iterations = 20000, nrChains = 1, message = TRUE, burnin = 10000) # for testing combinations 
...
saveRDS(out_all, file = paste0("./output/data/parameters/combinations/out_", "matboundaries60", ".rds"))
